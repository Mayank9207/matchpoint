...
export default function Profile() {
  ...
  const fetchUserStats = useCallback(async (overrideUser) => {
    try {
      ...
      if (userId) {
        const joinedMatchesCount = matches.filter((match) =>
          match.participants?.some(
            (p) => String(p.user?._id || p.user) === String(userId)
          )
        ).length;

        const createdMatchesCount = matches.filter(
          (match) => String(match.host?._id || match.host) === String(userId)
        ).length;
    ...
    try {
      ...
      if (userId) {
        const userMatches = matches.filter((match) => {
          ...
          const isCreator = String(match.host?._id || match.host) === String(userId);
        ...
        const recent = userMatches
          ...
          .map((match) => {
            ...
            const locationLabel =
              ...
              match.locationDetails?.city ||
              match.locationDetails?.locality ||
              "Location unavailable";

            return {
              id: match._id || match.id,
              sport: match.sport,
              action: isCreator ? "created" : "joined",
